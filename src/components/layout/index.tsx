...
import { Row as ReactRow, Col as ReactCol, mediaBreakpointDown } from 'react-grid'
...
export const Container = styled.div<{ wide?: boolean }>`
  ...
  margin: 0 auto;
  padding: 0 50px;
  max-width: ${({ wide }) => (wide ? theme.wideContainerWidth : theme.containerWidth)};
...
export const Col = (props) => <ReactCol {...props} css={styles} />
...
export const FormGroup: FunctionComponent<FromGroupProps> = ({
  ...
  <div className={className}>
    {label ? (
      <div css={[utils.display('flex'), utils.alignItems('center'), utils.mb(2)]} className={textContainerClassName}>
      ...
      <div css={[utils.display('flex'), utils.alignItems('center'), utils.mb(2)]}>
        <Text
          css={[
            utils.mr(3),
            css`
