...
import { FunctionComponent, TextareaHTMLAttributes, useState } from 'react'
...
const TextAreaWrapper = styled.div<{ width?: string; hasError?: boolean }>`
  position: relative;
  display: flex;
...
const FieldWrapper = styled.fieldset<{ hasError?: boolean }>`
  ...
  padding: 0 0.5em;
  min-height: 3.25em;
  height: ${({ hasError }) => (hasError ? 'calc(100% - 26px)' : '100%')};
...
const InputControl = styled.textarea<TextAreaProps>`
  font-size: 1em;
  width: 100%;
  resize: vertical;
