  ...
  return fetch(url, params)
    .then(async (res) => {
      if (res.ok) {
        ...
        const result = res.status === 401 ? { 'Message': 'Login Token Error' } : await res.json()
        throw { httpRes: res, data: result }
    ...
    .catch((e) => {
      if (e.httpRes.status === 401) {
        throw [new CustomError(CustomErrorType.SESSION_EXPIRED, e.data, e.data.Message)]
      }
      else {
        if (e.data.Error) {
          throw e.data.Error.map((err: any) => new CustomError(CustomErrorType.SERVICE_ERROR, err, err.Message))
        } else if (e.data?.DTOApplication[0]?.ValidationError) {
          throw e.data.DTOApplication[0].ValidationError.map((err: any) => new CustomError(CustomErrorType.SERVICE_ERROR, {
            Name: err.TypeCd,
