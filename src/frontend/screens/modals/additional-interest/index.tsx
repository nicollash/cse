...
import { AddInterestItem } from "~/frontend/screens/pages/customize/components";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faPlusCircle, faTimesCircle } from "@fortawesome/free-solid-svg-icons";

import { Text } from "~/frontend/components";

import { aiConfig } from "~/frontend/utils/configuration/additionalInterest";
...
export const AdditionalInterestModal: FunctionComponent<Props> = ({
  ...
  useEffect(() => {
    if (editingItem != null) {
      let position = additionalInterest.findIndex(
        (el) => el.id === editingItem.id
      );
    ...
    Yup.object().shape({
      partyInfo: Yup.object({
        addr: Yup.object({
          ...
          PostalCode: Yup.string().when("RegionCd", {
            is: "None",
            then: Yup.string()
              .required(messages.AIModal.Errors.RequiredZip)
              .matches(
                /(^\d{5}$)|(^\d{5}-\d{4}$)/,
                messages.AIModal.Errors.ZipFormat
              ),
            otherwise: Yup.string()
              .required(messages.AIModal.Errors.RequiredPostalCode)
              .matches(
                /(^\d{5}$)|(^\d{5}-\d{4}$)/,
                messages.AIModal.Errors.ZipFormat
              ),
          ...
          Addr2: Yup.string().when("RegionCd", {
            ...
            otherwise: Yup.string().required(
              messages.AIModal.Errors.AddressLine2Required
            ),
          }),
        }),
      }),
      interestName: Yup.string().required(
        formik.values.interestTypeCd == "Trust/ee"
          ? messages.AIModal.Errors.TrustRequired
          : messages.AIModal.Errors.NameRequired
      ),
      interestTypeCd: Yup.string().required(
        messages.AIModal.Errors.InterestTypeRequired
      ),
      legalLanguage: Yup.string().when("interestTypeCd", {
        is: "Trust/ee",
        then: Yup.string().required(
          messages.AIModal.Errors.LegalLanguageRequired
        ),
      }),
    });
              ...
              >
                <div css={[styles.row]}>
                  ...
                  <div css={[styles.row, styles.formGroup, utils.mb(3)]}>
                    <Text bold={true}>
                      {"Number " + formik.values.sequenceNumber}
                    </Text>
                  </div>
                  ...
                  >
                    <FormGroup
                      key="ai-interestType"
                      label={messages.AIModal.AddNewSection.InterestType}
                      css={[
                        styles.linearLines,
                        utils.mr(2),
                    ...
                    <FormGroup
                      ...
                      label={
                        formik.values.interestTypeCd == "Trust/ee"
                          ? messages.AIModal.AddNewSection.TrustLabel
                          : messages.AIModal.AddNewSection.NameLabel
                      }
                    ...
                    <FormGroup
                      key="ai-loanNumber"
                      label={messages.AIModal.AddNewSection.LoanLabel}
                      css={[utils.mr(2)]}
                      textContainerClassName="linearLabelWithValidation"
                    >
                      <FormikInput css={utils.mb(3)} name="accountNumber" />
                    </FormGroup>
                  </div>
                  ...
                  {formik.values.interestTypeCd == "Trust/ee" ? (
                    <div css={[utils.fullWidth, styles.formPadding]}>
                      <FormGroup
                        key="ai-legalLanguage"
                        label={
                          messages.AIModal.AddNewSection.LegalLanguageLabel
                        }
                        css={[utils.mr(2)]}
                        textContainerClassName="linearLabelWithValidation"
                      >
                        <FormikInput css={utils.mb(3)} name="legalLanguage" />
                      </FormGroup>
                    </div>
                  ) : (
                    ""
                  ...
                  <div css={utils.fullWidth}>
                    <FormGroup
                      key="ai-address"
                      label={messages.AIModal.AddNewSection.Address.Label}
                    ...
                    >
                      <div css={styles.linearLines}>
                        <FormikSelect
                          ...
                          placeholder={
                            messages.AIModal.AddNewSection.CountryPlaceholder
                          }
                      ...
                      <div css={styles.linearLines}>
                        <div css={styles.addressInputContainer}>
                          <FormikInput
                            css={[utils.mb(3)]}
                            placeholder={
                              messages.AIModal.AddNewSection.Address
                                .AddressPlaceholder
                            }
                      ...
                      <div css={styles.linearLines}>
                        {formik.values.partyInfo.addr.RegionCd === "" ||
                        formik.values.partyInfo.addr.RegionCd === "None" ? (
                          <div>
                            <div
                              css={[
                                styles.linearLines,
                                styles.cityStateContainer,
                                utils.hideOnMobile,
                              ]}
                            >
                              <FormikInput
                            ...
                            <div
                              css={[styles.linearLines, utils.visibleOnMobile]}
                            >
                              <FormikInput
                  ...
                  <FormGroup
                    key="ai-interest-in"
                    label={messages.AIModal.AddNewSection.HasAnInterestInLabel}
                    css={[styles.formGroup]}
                  >
                  ...
                  <div css={styles.saveButton}>
                    <Button type="submit">
                      {messages.AIModal.AddNewSection.AddItem}
                    </Button>
                  </div>
                </div>
              </form>
              ...
              additionalInterest
                ...
                .map((ai, index) => (
                  <AddInterestItem
                    ...
                    item={ai}
                    index={index}
                    vehicles={formik.values.linkReference}
                    onDelete={() => {
