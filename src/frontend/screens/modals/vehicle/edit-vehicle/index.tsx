...
export const EditVehicleModal: FunctionComponent<Props> = ({
  ...
  const customValidate = (values: VehicleInfo) => {
    ...
    if (
      values.symbolCode == "ZZXXXX" ||
      ((values.symbolCode == "" || !values?.symbolCode) &&
        (values.comprehensive != "None" ||
          values.collisionDeductible != "None"))
    ) {
      //Required
      if (!values.costNew) {
        //Add condition you want
  ...
  const formik = useFormik<VehicleInfo>({
    ...
    initialValues: {
      ...defaultValue,
      annualMileage: defaultValue?.annualMileage
        ? defaultValue.annualMileage
        : "Unverified",
      estimateAnnualMiles:
        defaultValue?.estimateAnnualMiles &&
        defaultValue?.estimateAnnualMiles != "0"
          ? defaultValue.estimateAnnualMiles
          : "13000",
      verifiedMileageOverride: defaultValue?.verifiedMileageOverride
        ? defaultValue.verifiedMileageOverride
        : "No",
    },
  ...
  return (
    <Modal
      ...
      title={
        defaultValue?.id ? messages.CarModal.EditCar : messages.CarModal.AddCar
      }
      ...
      <FormikProvider value={formik}>
        ...
        >
          ...
          <div css={styles.row}>
            ...
            {formik.values.symbolCode == "ZZXXXX" ||
            ((formik.values.symbolCode == "" || !formik.values?.symbolCode) &&
              (formik.values.comprehensive != "None" ||
                formik.values.collisionDeductible != "None")) ? (
              <FormGroup
            ...
            >
              <FormikSpinner
                ...
                onChange={(e) => {
                  ...
                  if (
                    formik.values.annualMileage == "Verified" &&
                    formik.values.recommendedMileage != e.toString()
                  ) {
                    formik.handleChange({
                      target: {
                        name: "verifiedMileageOverride",
          ...
          >
            ...
            <Button disabled={vinChanged || vinRequestError} type="submit">
