...
import { DriverPointsInfo } from "~/types";
import {
  Text,
  Row,
  Col,
...
export const AddDriverPointsItem: FunctionComponent<Props> = ({
  ...
  return (
    <div css={[styles.container, styles.aiItem]} className={className}>
      <div>
        <div
          onClick={() => !newItem && setCollapsed(!collapsed)}
          css={styles.heading}
        >
          <Text bold>{`#${item.infractionDt?.getFullYear() || ""} - ${
            item.sourceCd
          } - ${item.typeCd || ""}`}</Text>
                ...
                <Row>
                  <Col md={12} lg={12}>
                    <FormGroup
                      ...
                      css={[styles.linearLines, utils.mr(2)]}
                      textContainerClassName="linearLabelWithValidation"
                      containerClassName="customInfractionContainer"
                    >
                      <FormikSelect
                  ...
                  <Row css={[styles.aiItemAligment]}>
                    ...
                    >
                      <div css={[styles.infoItem]}>
                        ...
                        <Text size="1" css={utils.mx(1)}>
                          {item.infractionDt
                            ? `${parseDateToString(item.infractionDt)}`
                            : `-`}
                        </Text>
                      </div>
                    </Col>
                    ...
                    >
                      <div css={[styles.infoItem]}>
                        ...
                        <Text size="1" css={utils.mx(1)}>
                          {item.expirationDt
                            ? parseDateToString(item.expirationDt)
                            : `-`}
                        </Text>
                      </div>
                    </Col>
