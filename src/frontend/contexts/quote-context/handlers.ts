...
import {
  ...
  handleSSN,
  parseDriverPoint,
} from "../../utils/mapping/application";
...
import { explodeAddress, httpClient, logger } from "~/frontend/utils";
...
export const handleUpdateDriverPoints = async (
  ...
  const res = await updateDriverPoints(
    user.LoginId,
    parseDriverPoint(action, driverNumber, driverPoint),
    handleSSN(DTOApplication)
  );
...
export const handleConvertToApplication = async (
  ...
  const res = await convertQuoteToApplication(
    user.LoginId,
    handleSSN(DTOApplication)
  );
