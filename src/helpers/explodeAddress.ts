export const explodeAddress = (singleLineAddress, cb) => {
  ...
  addySplit.forEach(function (addyPart) {
    ...
    if (/[0-9]/.test(addyPart)) {
      return (
        !addressObj.street_address1 && (addressObj.street_address1 = addyPart)
      );
    }
    // if looks like state
...
export function implodeAddress(addressObj, cb) {
  ...
  var addyParts = [],
    addyPart;
  if (
    typeof addressObj.street_address1 == "string" &&
    (addyPart = addressObj.street_address1.trim())
  ) {
    addyParts[0] = addyPart;
    if (
      typeof addressObj.street_address2 == "string" &&
      (addyPart = addressObj.street_address2.trim())
    ) {
      addyParts[0] += " " + addyPart;
    }
  }
  ["city", "state"].forEach(function (addyKey) {
    if (
      typeof addressObj[addyKey] == "string" &&
      (addyPart = addressObj[addyKey].trim())
    ) {
  ...
  if (
    typeof addressObj.postal_code == "string" &&
    (addyPart = addressObj.postal_code.trim())
  ) {
    singleLineAddress += " " + addyPart;
    singleLineAddress = singleLineAddress.trim();
  }
  if (
    typeof addressObj.country == "string" &&
    (addyPart = addressObj.country.trim())
  ) {
