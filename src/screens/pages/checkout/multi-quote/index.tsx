...
export const CheckoutScreen: FunctionComponent<RouteProps> = () => {
  ...
  useEffect(() => {
    ...
    if(quoteDetail.planInfo[selectedPlan].paymentMethod != null && quoteDetail.planInfo[selectedPlan].paymentMethod.oneIncPaymentToken != undefined){
      setReducedPaymentMethodInfo(quoteDetail.planInfo[selectedPlan].paymentMethod)
      setIssuePolicy(true)
    }    
  ...
  useEffect(() => {
    if (paymentFrequency === 'full') {
      setAmountToPay((parseFloat(quoteDetail.planInfo[selectedPlan].fullPrice) + parseFloat(quoteDetail.planInfo[selectedPlan].writtenFeeAmt)).toFixed(2).toString())
    } else {
      setAmountToPay(
                ...
                >
                  <div css={[styles.formRow]}>
                        ...
                        <div css={[styles.paymentInformation.description]}>
                          <div>
                            <Text color={theme.color.primary}>
                              ${(parseFloat(quoteDetail.planInfo[selectedPlan].fullPrice) + parseFloat(quoteDetail.planInfo[selectedPlan].writtenFeeAmt)).toFixed(2)}
                            </Text>
                    ...
                    {paymentFrequency === PaymentOptions.PaymentFrequency.monthly && (
                      <div css={[styles.paymentInformation.description]}>
                        <div css={utils.mb(4)}>
                          <Text color={theme.color.primary}>{`$${quoteDetail.planInfo[selectedPlan].downPayment}`}</Text>
                          <Text css={utils.ml(3)}>{messages.Checkout.Today}</Text>
                          ...
                          <Text size="0.9em">
                              {
                                messages.Checkout.MonthlyDescription(
                                  quoteDetail.planInfo[selectedPlan].paymentSchedule.length - 1, 
                                  quoteDetail.planInfo[selectedPlan].paymentSchedule[1].BillAmt,
                                  amountToPay)
                              }
                          </Text>
                          <Text size="0.9em" bold>
                            {messages.Checkout.InstallmentFee(quoteDetail.planInfo[selectedPlan].installmentFee)}
                          </Text>
                        </div>
                      </div>
                    )}
