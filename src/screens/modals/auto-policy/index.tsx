...
import { hasSameValue, logger } from '~/utils'
        ...
        >
          <Row css={utils.alignItems('center')}>
            <Col lg={6} md={12}>
              ...
              <FormGroup label={messages.Coverage.UninsuredMotoristBodilyInjury}>
                <FormikSelect
            ...
            <Col lg={6} md={12} css={styles.pricing}>
              ...
              <div css={styles.dataItem}>
                ...
                <Text bold color={theme.color.primary}>
                  $ {formik.values.uninsuredMotoristLimit.includes('/') ? formik.values.uninsuredMotoristLimit.split('/')[1] : formik.values.uninsuredMotoristLimit}
                </Text>
              </div>
              <div css={styles.dataItem}>
              ...
              <div css={styles.dataItem}>
                <Text css={styles.dataLabel}>{messages.Coverage.UM_PD_WCD_Applies}</Text>
                <Text bold color={theme.color.primary}>
                  ...
                  Content: () => (
                    <div css={[utils.display('flex')]}>
                      ...
                      <Slider css={styles.slider} {...sliderSettings}>
                        {formik.values.vehicleInfo.map((info, key) =>
                        (
                          info.status === 'Active' &&
                          <CarCoverage
                            formik={formik}
                            ...
                            onChange={(e) => {
                              formik.handleChange(e)
                              logger('--- update individual car coverage ---', key, e)
                            }}
                          />
                          //: ''
...
const getInitialValue = (defaultValue) => ({
  ...defaultValue,
  applyToAll: true,
  coverageInfo: defaultValue.vehicleInfo.length > 0 ? {
    status: hasSameValue(
    ...
    collisionDeductible: hasSameValue(
      defaultValue.vehicleInfo.map((vehicle) => vehicle.collisionDeductible),
    )
      ? defaultValue.vehicleInfo[0].collisionDeductible
      : '',
    dailyRentalCarLimit: hasSameValue(
      defaultValue.vehicleInfo.map((vehicle) => vehicle.dailyRentalCarLimit),
    )
      ? defaultValue.vehicleInfo[0].dailyRentalCarLimit
      : '',
    medicalPartsAndAccessibility: hasSameValue(
      defaultValue.vehicleInfo.map((vehicle) => vehicle.medicalPartsAndAccessibility),
    )
      ? defaultValue.vehicleInfo[0].medicalPartsAndAccessibility
      : '',
    roadsideAssistance: hasSameValue(
      defaultValue.vehicleInfo.map((vehicle) => vehicle.roadsideAssistance),
    )
      ? defaultValue.vehicleInfo[0].roadsideAssistance
      : null,
    fullGlass: hasSameValue(defaultValue.vehicleInfo.map((vehicle) => vehicle.fullGlass))
    ...
    originalReplacementCost: hasSameValue(
      defaultValue.vehicleInfo.map((vehicle) => vehicle.originalReplacementCost),
    )
