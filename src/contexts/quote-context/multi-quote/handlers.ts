...
export const handleExternalApplicationCloseOut = async (
  ...
  const res = await externalApplicationCloseOut(
    user.LoginId,
    getUpdatedDTOApplication(quoteResponse, newQuote).find(dtoApp => dtoApp.DTOApplicationInfo[0].IterationDescription == selectedPlan.toString().toUpperCase()),
  )
...
export const handleAddVehicle = async (
  ...
  const res = await addVehicle(newRisk, DTOApplication)
...
export const handleAddDriver = async (
  ...
  const res = await addDriver(newDriver, DTOApplication)
...
export const handleUpdateDriverPoints = async (
  ...
  const res = await updateDriverPoints(user.LoginId, parseDriverPoint(action, driverNumber, driverPoint), DTOApplication)
...
export const handleConvertToApplication = async (
  ...
  const updatedApplication = res.DTOApplication.find(
    (application: DTOApplication) =>
      application.DTOApplicationInfo[0].IterationDescription ===
      DTOApplication[0].DTOApplicationInfo[0].IterationDescription,
  )
